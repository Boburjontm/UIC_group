  ...
  return (
    <div className="bg-[#1e1e20] pt-[100px]">
      <div className="max-w-[1200px] mx-auto px-5">
        ...
        <div className="flex flex-wrap py-[30px] gap-4">
          {categories.map((cat) => (
            <motion.div
              className=""
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-8">
          <input
            type="text"
            placeholder="Search..."
            className="w-full sm:w-1/2 p-3 rounded-[10px] bg-[#171717] text-white"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
          />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-10 gap-x-16 pt-[30px]">
          {currentItems.map((item, index) => (
            <motion.div
              whileInView={{ translateX: ["-30px", 0], opacity: [0.9, 1] }}
              transition={{ duration: 1 }}
              key={index}
              className="group flex flex-col sm:flex-row gap-5 h-auto sm:h-[220px] bg-[#2b2b2e] rounded-md"
            >
              <div className="relative w-full sm:w-[220px] flex-[.4]">
                <img
                  className="absolute -top-5 sm:static duration-300 transition-all rounded-lg left-0 sm:left-5 sm:group-hover:left-0 sm:group-hover:top-0 w-[210px] h-[210px] object-cover"
                  src={item.img}
                  alt=""
                />
              </div>
              <div className="py-[20px] pr-[20px] flex-[.6]">
                ...
                <p className="font-normal text-xs !leading-140 text-[gray] line-clamp-2 sm:line-clamp-3 mt-2 mb-3">
                  {item.desc}
                </p>
